...
def _gen_fourier_features(
    ...
    ) # this should be 10, and not 16
    weight = weight.reshape(*kernel.batch_shape, num_outputs // 2, num_inputs)
...
def _gen_kernel_features_scale(
    ...
    *,
    ...
    #if isinstance(kernel.base_kernel, (AdditiveKernel, ScaleKernel)):
    #    
    #    kern = kernel.base_kernel.kernels[0]
#
...
#
    ...
    if isinstance(feature_map, AdditiveKernelFeatureMap):
        feature_map.output_transform = OutputscaleTransform(kernel)
    else:
        feature_map.output_transform = ChainedTransform(
            OutputscaleTransform(kernel), feature_map.output_transform
        )
...
@GenKernelFeatures.register(kernels.AdditiveKernel)
def _gen_kernel_features_additive(
    ...
    *,
    ...
    for kern in kernel.kernels:
        fourier_features.append(GenKernelFeatures(
            kern,
            num_inputs=num_inputs,
            num_outputs=num_outputs,
        ))
    return AdditiveKernelFeatureMap(fourier_features)
