...
class AdditiveKernelFeatureMap(FeatureMap):
    def __init__(
        self,
        maps: Optional[Iterable[KernelFeatureMap]] = None,
    ) -> None:
        r"""Adds KernelFeatureMaps to a list (for additive kernels specifically).

        Args:
            maps: The KernelFeatureMaps.
        """
        super().__init__()
        self.maps = maps

    def forward(self, x: Tensor, **kwargs: Any) -> Union[Tensor, List[Tensor]]:
        out = torch.stack([kernel_map(x) for kernel_map in self.maps], dim=0)
        return out

    @property
    def batch_shape(self) -> Size:
        return self.maps[0].batch_shape
    
    @property
    def num_outputs(self) -> int:
        return self.maps[0].num_outputs
