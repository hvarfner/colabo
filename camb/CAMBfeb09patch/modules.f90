...
!!!! BR09_LRG additions
        !! modified version of Transfer_GetMatterPower(), but adds outputs for P_{no wiggles} and P_{halofit, no wiggles}

        subroutine Transfer_GetMatterPowerAndNW(MTrans,outpower, itf, in, minkh, dlnkh, npoints, kmindata, getabstransferscale, outpowernw, outpowerrationwhalofit)
          ...
          !! allocate a separate transfer data object for the 'no wiggles' transfer function
          ...
          !! smooth spline algorithm.
          !! first fill in the inputs to bspline and initialize Transnw
          !! then call bspline
          !! then fill in transfers for no wiggles.
          ...
          !! now just fill in the outpowers, exactly as in Transfer_GetMatterPower()
            ...
            do while (lastix <= npoints)
               ...
               outpowerrationwhalofit(lastix) = 2*outpowerrationwhalofit(lastix-1) - outpowerrationwhalofit(lastix-2)
               lastix = lastix+1
            end do
            ...
            do il = 1, npoints
               k = exp(logmink + dlnkh*(il-1))*h
               outpower(il) = outpower(il) * ScalarPower(k,in)
               outpowerrationwhalofit(il) = outpowerrationwhalofit(il)/outpowernw(il)  !! do this first because the ScalarPower calls cancel.
        ...
        !! currently written for one InitPower
        subroutine Transfer_SaveMatterPowerforLRG(MTrans, FileNames, my_w_lam)
          use IniFile
          !Export files of linear, no-wiggles, and nwhalofit matter power spectra in h^{-1} Mpc units, against k/h.
          Type(MatterTransferData), intent(in) :: MTrans
          character(LEN=Ini_max_string_len), intent(IN) :: FileNames(*)
          integer itf,in,i
          !! BR09_LRG hard-coded values in CosmoMC LRG version
          integer ::  num_matter_power = 300
          real ::  matter_power_minkh =  0.999e-4  !minimum value of k/h to store
          real ::  matter_power_dlnkh = 0.03     !log spacing in k/h
          real(dl) :: kmindata = 0.02
          real(dl) :: getabstransferscale
          real(dl), intent(in) :: my_w_lam

          real, dimension(:,:), allocatable :: outpower, outpowernw, outpowerrationwhalofit
          character(LEN=80) fmt
          Type(MatterPowerData) :: PK_data, PKnw_data, PKnwhalofit_data

          if (CP%InitPower%nn > 1) stop 'SaveMatterPowerforLRG only does one initial power'
          write (fmt,*) 4
          !write (fmt2,*) 1
          fmt = '('//trim(adjustl(fmt))//'E15.5)'
          !fmt2 = '('//trim(adjustl(fmt2))//'E15.5)'
          do itf=1, CP%Transfer%num_redshifts
            if (FileNames(itf) /= '') then
              if (.not. transfer_interp_matterpower ) stop 'SaveMatterPowerforLRG only uses interpolation.'

              allocate(outpower(num_matter_power, CP%InitPower%nn))
              allocate(outpowernw(num_matter_power, CP%InitPower%nn))
              allocate(outpowerrationwhalofit(num_matter_power, CP%InitPower%nn))
              do in = 1, CP%InitPower%nn
                call Transfer_GetMatterPowerAndNW(MTrans,outpower(1,in), itf, in, matter_power_minkh,matter_power_dlnkh, num_matter_power, kmindata, getabstransferscale, outpowernw,outpowerrationwhalofit)
                if (CP%OutputNormalization == outCOBE) stop 'outCOBE not supported.'
              end do

              open(unit=fileio_unit,file=FileNames(itf),form='formatted',status='replace')
              !! put in the header the abstransferscale and Cosmo parameters needed to compute the scale factor.
              write (fileio_unit, fmt) getabstransferscale, CP%omegak, CP%omegav, my_w_lam
              do i=1,num_matter_power
                write (fileio_unit, fmt) matter_power_minkh*exp((i-1)*matter_power_dlnkh),outpower(i,1:CP%InitPower%nn), outpowernw(i,1:CP%InitPower%nn), outpowerrationwhalofit(i,1:CP%InitPower%nn)
              end do
              close(fileio_unit)
              deallocate(outpower)
              deallocate(outpowernw)
              deallocate(outpowerrationwhalofit)
            end if  !! if Filenames(itf) /= ''
          end do

        end subroutine Transfer_SaveMatterPowerforLRG
